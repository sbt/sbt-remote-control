...
fullResolvers <<= (fullResolvers, bootResolvers) map {
  case (rs, Some(boot)) =>
    // Here we add any local filesystem repos we find, to ensure speedy updates repo (as first checked)
    val newRepos = boot collect {
      case x: FileRepository if !rs.exists(_.name == x.name) => x
    }
    newRepos ++ rs
  case (rs, _) => rs
}
