...
class CanRunMainPlay22Sbt13Project extends SbtProcessLauncherTest {
  val dummy = utils.makeEmptySbtProject("runMainPlay22", TestUtil.sbt13TestVersion)
...
""")
  ...
  try {
    ...
    val testActor = system.actorOf(Props(new Actor with ActorLogging {
      ...
      child ! RunRequest(sendEvents = true, mainClass = Some("play.core.server.NettyServer"))
      ...
      def receive: Receive = {
        ...
        case x: ErrorResponse =>
          result.failure(new AssertionError("Got ErrorResponse: " + x.error))

        case Status.Failure(e) =>
          result.failure(new AssertionError("Got exception", e))
