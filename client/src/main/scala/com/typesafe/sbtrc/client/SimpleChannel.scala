...
final private class SimpleSbtChannel(override val uuid: java.util.UUID,
  ...
  object thread extends Thread {
    override def run(): Unit = {
      ...
      while (running) {
        try {
          // TODO TODO TODO we need the serializations passed in to us from the client.
          val envelope = protocol.Envelope(socket.receive(), ImmutableDynamicSerialization.defaultSerializations)
